...
Redis now also maintains a BullMQ-style queue `marker` zset inside the same Lua
...
`addDelayMarkerIfNeeded` wake-up mechanism. Lane's current Rust worker still
uses polling `claim_next()` loops, but the Redis marker state is in place for a
future blocking worker path without requiring a second source of truth.
