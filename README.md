...
`update_data()` replaces a retained job payload, `save_stacktrace()` stores
retained failure stack traces and a failure reason, `add_log()` appends retained job logs, and
